 ...
 * @author jay.wu
 */
public class SimpleFilterService<C> implements FilterService<C> {

    private final Log log = LogFactory.get();

    private FilterChain<C> filterChain;

    public SimpleFilterService() {
    }

    public SimpleFilterService(Filter<C>... filters) {
        this(Arrays.asList(filters));
    }

    public SimpleFilterService(Class<? extends Filter<C>> filterClass) {
        this(ServiceLoaderUtil.loadAvailableList(filterClass));
    }

    public SimpleFilterService(List<? extends Filter<C>> filters) {
