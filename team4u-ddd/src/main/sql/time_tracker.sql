...
(
    ...
    `description`                  VARCHAR(10000)      NOT NULL DEFAULT '' COMMENT '上下文描述',
    `process_informed_of_timeout`  TINYINT(1)          NOT NULL DEFAULT 0 COMMENT '是否完成所有超时通知',
    `process_timed_out_event_type` VARCHAR(100)        NOT NULL DEFAULT '' COMMENT '超时时间类型',
    `timeout_occurs_on`            TIMESTAMP           NOT NULL DEFAULT '1970-01-01 23:59:59' COMMENT '超时时间',
    `retry_strategy_id`            VARCHAR(100)        NOT NULL DEFAULT '' COMMENT '重试策略标识',
    `retry_count`                  BIGINT(20) unsigned NOT NULL DEFAULT 0 COMMENT '已重试次数',
